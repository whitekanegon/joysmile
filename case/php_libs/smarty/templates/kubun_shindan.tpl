...
{include file="{$smarty.const._SITE_ROOT}lib/ssi/header.html"}
    {if $shindan_id eq 'others'}
    {$bcname = 'その他'}
    {else}
    {$bcname = $data.name}
    {/if}
<div id="topicpath"><a href="/">トップページ</a> &gt; <a href="/case/">症例紹介</a> &gt; <a href="/case/kubun.php?id=1">前期のみ</a> &gt; {$bcname}</div>
<div id="container">
...
{/if}
 ...
 <!--/[cont_header]-->
   ...
   {/foreach}
   {if $shindan_cate_flag}
   <h2 class="htype02">症例</h2>
   <dl class="categoy_entries kubun_shindan">
   {foreach from=$shindan_cate key="key" item="row_cate"}
   {if $row_cate}
    {if $shindan_id eq 'others'}
    <dt>【その他】</dt>
...
<p class="backtop"><a href="#" onClick="history.back(); return false;" class="cmnbtn03">前のページに戻る</a></p>
  {else}
