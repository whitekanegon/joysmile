...
<div style="margin-bottom:10px; padding:0 10px;">
   {if $row_cate}
    <div><strong>【{$category_title.$key.name}】</strong></div>
       {foreach from=$row_cate item="row" name="shindan"}
       <a href="detail.php?id={$row.id}&nendai={$nendai_id}&car=mb" style="line-height:1.8;">{$row.kansou_mark}{$row.age}歳 {$row.sex}{$row.sex_ex}</a>{if $smarty.foreach.shindan.last}{else}{if $smarty.foreach.shindan.index % 2 == 0} ／{else}<br />{/if}{/if}
       {/foreach}
    {/if}
</div>
    {/foreach}
