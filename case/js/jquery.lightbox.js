...
(function($) {
	...
	$.fn.lightBox = function(settings) {
		...
		function _start(objClicked,jQueryMatchedObj) {
			// kaneda custom!!
			settings.minus_count = objClicked.getAttribute('minus_count');
			if(settings.minus_count){
				light_box_finish();
			}
			...
			// kaneda custom!!
			if(settings.minus_count){
				for(var i=0; i<settings.minus_count; i++){
					settings.imageArray.pop();
				}
			}
		...
		function _show_image_data() {
			...
			if ( settings.imageArray[settings.activeImage][1] ) {
				//kaneda custom!!
				var rel = settings.imageArray[settings.activeImage][1];
				var cross_flag = rel.indexOf("lightbox");
				var setcap = $('#lightbox-image-details-caption').html(rel);
				if(cross_flag != -1){
					setcap.show(lightbox_load_cross()); 
				}else{
					setcap.show(); 
				}
				//kaneda custom!!
			...
			if ( settings.imageArray.length > 1 ) {
				$('#lightbox-image-details-currentNumber').html(settings.txtImage + ' ' + ( settings.activeImage + 1 ) + ' ' + settings.txtOf + ' ' + settings.imageArray.length + ' minus='+settings.minus_count).show();
			}		
		}
		/**
		...
		function _set_navigation() {
			...
			//kaneda customs!!
					...
					$('.lightboxCross')
						.unbind()
						.bind('click',function() {
							$kagen = $(this).attr("href").split(":");
							if($kagen[0] == '-'){
								settings.activeImage = settings.activeImage - eval($kagen[1]);
							}else{
								settings.activeImage = settings.activeImage + eval($kagen[1]);
							}
							_set_image_to_view();
