...
foreach($_GET as $key => $row){
	...
	elseif($key == "freeword" && $row){
		...
		$keyword = explode(" ", $row);
		$sql_freeword = db_tbl_alljoin($keyword,$s_con,0,1);
...
if(!$get_count){
	...
	$sql = "select * from sh_main order by id";
...
foreach($data as $key => $val){
	//preg_match("/^[_a-zA-Z0-9]*/" ,$data[$key]["thisinfo"], $match);
	$data[$key]["thisinfo"] = mb_substr($data[$key]["thisinfo"],0,50,"utf-8")."...";
}
...
$smarty -> assign('sql',$sql_freeword);
...
$smarty->display('admin_search_result.tpl');
